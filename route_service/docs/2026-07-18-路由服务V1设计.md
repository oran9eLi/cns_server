...
命令状态转换仍使用“期望旧状态 + 目标新状态”的条件更新。若 PostgreSQL 返回状态已变化，route_service 重新读取当前命令记录并收敛：当前状态已经达到期望或更晚终态时不反转；当前状态仍可合法推进时只重试一次受控转换；真实数据库不可用和非法状态迁移继续记录错误。
...
命令正常链路增加两类 `INFO` 业务日志：MQTT QoS 发布完成且命令状态成功持久化为 `dispatched` 后，记录可信 `source_id`、解析后的具体命令及 `学校名 / 设备内部编号（vendor_id）`；设备 ACK 解析并成功持久化、来源 ACK 发布回调已提交后，记录同一设备、业务状态及回程来源。配置命令只按固定协议字段顺序显示实际变更项，多项使用中文逗号分隔；飞控命令显示动作名，`set_motor_pwm` 同时显示四路 PWM。幂等状态回放、发布失败、非法或无法关联的 ACK 不输出成功日志，信息日志异常不得影响命令状态机或 ACK 回程。成功日志禁止包含 `request_id`、`command_id`、原始 JSON 或完整 MQTT topic。

当前匿名 MQTT 1883 只适合联调。正式部署已确认使用 CNS 私有 CA、端到端 MQTT TLS `8883`、每设备独立凭据和最小权限 ACL；云服务器 frps 只做 TCP 透传，现场 frpc 转发到回环地址的 Mosquitto。`command_sources` 继续承担业务来源白名单，不能替代 Broker 身份认证。完整部署与验收边界见 `2026-07-21-MQTT公网安全接入与FRP部署设计.md`。
