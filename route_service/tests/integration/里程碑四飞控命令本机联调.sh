...
usage() {
  echo "用法：$0 --config <测试配置> --migrations <迁移目录> --broker-host <地址> --broker-port <端口> --binary <程序> --database-url <测试库连接> --source-id <固定来源> --device-id <测试设备> [--control-timeout <秒>] [--check-only]"
}
...
[[ "$device_id" =~ ^[A-Za-z0-9]{20}$ ]] || { echo "错误：device_id 必须为 20 个 ASCII 字母或数字" >&2; exit 2; }
...
cleanup() {
  ...
  if ((device_preexisting == 0)); then
    ...
    psql "$database_url" -X -v ON_ERROR_STOP=1 -v device_id="$device_id" \
      -v school_name="$school_name" >/dev/null 2>&1 <<'SQL' || true
DELETE FROM command_sources WHERE source_id = :'device_id' AND device_id = :'device_id';
DELETE FROM device_latest_states WHERE device_id = :'device_id';
...
publish_request() {
  ...
  mosquitto_pub "${MQTT[@]}" -q 2 -t "$topic_namespace/sources/$source_id/control/request" \
    -m "$(jq -cn --arg request_id "$target_request" --arg device_id "$device_id" \
      '{schema_version:1,request_id:$request_id,target:{device_id:$device_id},command:"set_motor_pwm",parameters:{pwm_us:[1000,1100,1200,1300]}}')"
}
...
"$binary" --config "$config" --migrations "$migrations" --migrate-only
device_preexisting=$(psql "$database_url" -X -Atq -c "SELECT count(*) FROM devices WHERE device_id = '$device_id';")

mosquitto_pub "${MQTT[@]}" -q 2 -r -t "$topic_namespace/$device_id/registration" \
  -m "$(jq -cn --arg device_id "$device_id" --arg school_name "$school_name" --arg dcdw_label "$dcdw_label" \
    '{schema_version:3,device_id:$device_id,device_type:"cns_box",school_name:$school_name,dcdw_label:$dcdw_label,status:"online"}')"
start_service
deadline=$((SECONDS + 15))
until [[ $(psql "$database_url" -X -Atq -c "SELECT count(*) FROM devices WHERE device_id = '$device_id';") == 1 ]]; do
...
source_subscriber_pid=$!
mosquitto_sub "${MQTT[@]}" -q 2 -t "$topic_namespace/$device_id/control/set" >"$work_dir/device-set.jsonl" &
device_subscriber_pid=$!
sleep 1
...
mosquitto_pub "${MQTT[@]}" -q 2 -t "$topic_namespace/$device_id/control/ack" \
