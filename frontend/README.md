...
- 标准原生 WebSocket 接收实时设备状态、按设备实时遥测和命令结果。
...
- 主控箱和 PX4 详情页共用 `telemetry.subscribe` / `telemetry.realtime`；同一绘制周期只应用最后一帧，2 秒无实时帧后回退最新快照。
- 前端不发起 PX4 专属 RTT 探测，也不依赖 PX4 专属实时协议。
...
- 设备列表、主控箱详情、PX4 详情、REST 快照、WebSocket 状态和命令链路已经实现。
- 主控箱和 PX4 已接入统一实时遥测链路；实时帧只用于当前设备页面，不写数据库。
- 页面采用明亮、专业、现代的企业级风格，主色为 `#132B88`，使用公司现有透明 Logo。
