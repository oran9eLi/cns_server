...
当前状态：里程碑一至四已完成并合入 `main`。2026-07-28 已将 PostgreSQL、Mosquitto、route_service、backend_service、前端和 Nginx 部署到硬件服务器 `192.168.11.3`，服务端构建、迁移、数据恢复、systemd、自启、HTTP、数据库权限和 retained 快照均已验证；树莓派已通过公网 `112.124.52.232:1883` 和硬件服务器 `frpc.service` 接入，真实 registration、telemetry、数据库在线状态和后端详情接口已补验，命令 ACK 闭环仍待补验。里程碑五正式安全接入仍按私有 CA、MQTT TLS、每设备独立凭据、最小权限 ACL，以及云端 frps 到现场 frpc 的 TCP 透传设计继续实施；详细设计见 `route_service/docs/2026-07-21-MQTT公网安全接入与FRP部署设计.md`。
